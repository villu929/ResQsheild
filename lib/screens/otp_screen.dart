    ...
    return Scaffold(
      ...
      body: LayoutBuilder(
        builder: (context, constraints) {
          ...
          // OTP geometry MUST exactly match the six stones painted by
          // RealisticOceanPainter.  The numbers are therefore locked to
          // these six containers at every screen size.
          final otpBaseCy = h * 0.637;
          const boxWidth = 33.0;
          const boxHeight = 39.0;
          const boxGap = 9.0;
          const boxCount = 6;
          final totalWidth = boxWidth * boxCount + boxGap * (boxCount - 1);
          final startX = cx - totalWidth / 2;
          final startY = otpBaseCy - 30;

          // Keep the existing button positioning behaviour unchanged and safe against keyboard / small heights.
            ...
            children: [
              ...
              AnimatedBuilder(
                ...
                builder: (_, _) {
                  ...
                  final cy = otpBaseCy + floatY - h * 0.015;
                      ...
                      children: [
                        for (int i = 0; i < boxCount; i++)
                          Positioned(
                            ...
                            top:
                                (otpBaseCy + floatY - h * 0.015) -
                                30 +
                                _otpStoneOffset(i).dy,
                            width: boxWidth,
                            height: boxHeight,
                            child: Padding(
    ...
    return ClipRRect(
      ...
      child: Stack(
        ...
        children: [
          ...
          // The TextField occupies EXACTLY the same 33x39 container as
          // the painted stone. ClipRRect guarantees the input/cursor cannot
          // visually escape that stone, even when the screen size changes.
          TextField(
      ...
      children: [
        ...
        AnimatedBuilder(
          ...
          builder: (context, child) {
            ...
            // Soft but clearly visible 3-axis floating tilt.
            // X + Y + Z are slightly stronger so the platform reads as a
            // real floating object, while the layered slow sine waves keep
            // the motion smooth, soft and non-mechanical.
            final tiltX =
                -(math.pi * 43.0 / 180) +
                math.sin(t * 0.472 + 0.6) * (math.pi * 5.20 / 180) +
                math.sin(t * 0.2917 + 1.5) * (math.pi * 1.25 / 180) +
                math.sin(t * 0.1803 + 2.1) * (math.pi * 0.42 / 180);

            final tiltY =
                math.sin(t * 0.347 + 1.15) * (math.pi * 3.15 / 180) +
                math.sin(t * 0.2113 + 2.0) * (math.pi * 1.05 / 180) +
                math.sin(t * 0.1327 + 0.4) * (math.pi * 0.34 / 180);

            final tiltZ =
                math.sin(t * 0.3817 + 0.4) * (math.pi * 2.75 / 180) +
                math.sin(t * 0.2361 + 1.2) * (math.pi * 0.95 / 180) +
                math.sin(t * 0.1459 + 0.8) * (math.pi * 0.32 / 180);
...
class LoginRipplePainter extends CustomPainter {
  ...
  // Fewer, evenly-spaced rings so the water reads as one continuous
  // ripple train instead of several ripples appearing at once.
  static const int ringCount = 11;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Slower overall motion. One complete ripple cycle takes much longer,
    // giving the rings time to separate naturally.
    final time = elapsed * math.pi * 2.0 * 0.38;

    final center = Offset(size.width * 0.5, size.height * 0.5);

    for (int ring = 0; ring < ringCount; ring++) {
      // All rings use the same slow travel speed and evenly-spaced phases.
      // This removes the old random bunching of 5-6 ripples together.
      const speed = 0.045;
      final raw = elapsed * speed + ring * 0.100;
      ...
      // Slightly smaller maximum expansion keeps the ripple subtle.
      final radius = 18.0 + eased * 172.0;
      final ry = radius * aspect;

      // Fade each ring naturally as it expands.
      final opacity = (0.145 * envelope * (1.0 - phase * 0.10)).clamp(
        0.0,
        0.17,
      );
      ...
      for (int i = 0; i <= pointsPerRing; i++) {
        ...
        // Gentle irregularity: enough to avoid a perfect geometric ellipse,
        // but much less distortion than before.
        // Uneven, slightly lopsided water shape instead of a clean circle.
        final largeWave = math.sin(a * 2.0 + ring * 1.25 + time * 0.12) * 0.070;
        final largeWave2 =
            math.sin(a * 3.0 - ring * 0.63 + time * 0.075 + 1.3) * 0.040;
        final mediumWave =
            math.sin(a * 5.0 + ring * 1.47 - time * 0.08) * 0.026;
        final fineWave = math.sin(a * 8.0 - ring * 0.91 + time * 0.05) * 0.014;

        // Fixed per-ring irregularity keeps each ripple shape different
        // without making the outline jitter from frame to frame.
        final irregular =
            math.sin(a * 2.7 + ring * 2.41) * 0.018 +
            math.sin(a * 6.3 - ring * 1.17 + 0.8) * 0.010;

        final organic =
            1.0 + largeWave + largeWave2 + mediumWave + fineWave + irregular;

        final finalRadius = radius * organic;
      ...
      canvas.drawPath(
        ...
        Paint()
          ..color = Colors.white.withOpacity(opacity)
          ..style = PaintingStyle.stroke
          ..strokeWidth = (1.25 - phase * 0.25).clamp(0.82, 1.25)
          ..strokeCap = StrokeCap.round
          ..strokeJoin = StrokeJoin.round,
      );

      // Very subtle soft halo follows the same ring, so the ripple feels
      // like light spreading through water rather than a hard outline.
      if (phase > 0.06) {
        final softPath = Path();
        final softRadius = radius + 2.0;
        ...
        for (int i = 0; i <= pointsPerRing; i++) {
          ...
          final softDistortion =
              1.0 +
              math.sin(a * 2.0 + ring * 1.4 + time * 0.10) * 0.040 +
              math.sin(a * 3.0 - ring * 0.7 - time * 0.06) * 0.022 +
              math.sin(a * 6.0 + ring * 0.9 + time * 0.04) * 0.010;
        ...
        canvas.drawPath(
          softPath,
          Paint()
            ..color = Colors.white.withOpacity(opacity * 0.13)
            ..style = PaintingStyle.stroke
            ..strokeWidth = 0.40
            ..strokeCap = StrokeCap.round
            ..strokeJoin = StrokeJoin.round
    ...
    // Subtle secondary ripples add smaller overlapping waves for a realistic
    // water-surface feel without making the animation look crowded.
    const int secondaryCount = 6;
    for (int ring = 0; ring < secondaryCount; ring++) {
      final raw =
          elapsed * (0.058 + ring * 0.006) +
          ring * 0.19 +
          math.sin(ring * 4.73) * 0.08;
      var phase = raw - raw.floorToDouble();
      if (phase < 0.0) phase += 1.0;

      final eased = _smoother(phase);
      final envelope = math.sin(phase * math.pi);
      final radius = 35.0 + eased * 185.0;
      final opacity = (0.050 * envelope).clamp(0.0, 0.055);
      if (opacity < 0.002) continue;

      final path = Path();
      for (int i = 0; i <= 64; i++) {
        final a = i / 64.0 * math.pi * 2.0;
        final distortion =
            1.0 +
            math.sin(a * 3.0 + ring * 1.8 + time * 0.17) * 0.045 +
            math.sin(a * 7.0 - ring * 1.1 - time * 0.11) * 0.020 +
            math.sin(a * 2.6 + ring * 2.4) * 0.012;

        final x = center.dx + math.cos(a) * radius * distortion;
        final y = center.dy + math.sin(a) * radius * aspect * distortion;
    ...
    // One slow contact ripple around the button.
    // It stays subtle so it doesn't create the old "sudden 5-6 ripples"
    // appearance.
    final contactRaw = elapsed * 0.30 + rippleKey * 0.31;
    ...
    final contactRadius = 34.0 + contactEase * 142.0;
    ...
    for (int i = 0; i <= 72; i++) {
      ...
      final wobble =
          1.0 +
          math.sin(a * 2.0 + time * 0.11) * 0.045 +
          math.sin(a * 3.0 - time * 0.08 + 1.7) * 0.022;
    ...
    canvas.drawPath(
      contactPath,
      Paint()
        ..color = Colors.white.withOpacity(0.105 * contactFade)
        ..style = PaintingStyle.stroke
        ..strokeWidth = (1.20 - contactPhase * 0.25).clamp(0.80, 1.20)
        ..strokeCap = StrokeCap.round
        ..strokeJoin = StrokeJoin.round,
    );

    // Tap ripple: slower and softer, with a single continuous expansion.
    if (rippleKey > 0) {
      final tapRaw = elapsed * 0.34 + rippleKey * 0.41;
      ...
      final tapRadius = 28.0 + tapEase * 152.0;
      ...
      for (int i = 0; i <= 68; i++) {
        ...
        final wobble =
            1.0 +
            math.sin(a * 2.0 + time * 0.12) * 0.045 +
            math.sin(a * 4.0 - time * 0.08 + 2.1) * 0.022;
      ...
      canvas.drawPath(
        tapPath,
        Paint()
          ..color = Colors.white.withOpacity(0.14 * tapFade)
          ..style = PaintingStyle.stroke
          ..strokeWidth = (1.40 - tapPhase * 0.25).clamp(0.88, 1.40)
          ..strokeCap = StrokeCap.round
          ..strokeJoin = StrokeJoin.round,
      );
    }
